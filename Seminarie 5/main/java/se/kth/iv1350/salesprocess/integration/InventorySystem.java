...
public class InventorySystem {
     ...
     * If barcode is a hardcoded specifiec value the database exception is called.If barcode doesn't exist the invalid itemidentifier is called.
     ...
     */
    public ItemDTO getItemFromInventorySystem(int barcode) throws DataBaseException, InvalidItemIdentifierException{
        if(barcode == 25)
            throw new DataBaseException("Data base can't be reached!");    
        int x = 0;
        while(itemList.get(x).getItemIdentifier() != barcode){
            x++;
            if(x >= itemList.size())
                throw new InvalidItemIdentifierException(barcode);
        }
